...
function isDoubleRangeTypeConstraintsDTO(o: unknown): o is DoubleRangeDTO {
  return (
    ...
    '__brand' in o &&
    typeof o.__brand == 'string' &&
    o.__brand == 'DoubleRange' &&
    'type' in o &&
    typeof o.type == 'string' &&
    o.type == TypeDTO.DoubleType &&
    'min' in o &&
    typeof o.min == 'number' &&
    'max' in o &&
    typeof o.max == 'number'
...
function isNoneTypeConstraintsDTO(o: unknown): o is NoneDTO {
  return (
    ...
    'type' in o &&
    typeof o.type == 'string' &&
    Object.values(TypeDTO).some((t) => t == o.type)
  )
}

export const typeConstraintsDeserializer = Deserializer<
  TypeConstraintsDTO,
  TypeConstraints<unknown>
>(
  isTypeConstraintsDTO,
  (dto) => {
    if (isIntRangeTypeConstraintsDTO(dto)) {
      return IntRange(dto.min, dto.max)
    } else if (isDoubleRangeTypeConstraintsDTO(dto)) {
      return DoubleRange(dto.min, dto.max)
    } else if (isEnumTypeConstraintsDTO(dto)) {
      return Enum(new Set(dto.values))
      ...
      throw DeserializeError(
        'It was not possible to serialize a TypeConstraintsDTO into any of [IntRangeDTO, DoubleRangeDTO, EnumDTO, NoneDTO]'
      )
    }
  },
  (obj) =>
    `Unable to deserialize ${obj} into a TypeConstraints since it was not a TypeConstraintsDTO`
)
