    ...
    <div>
      <div class="relative">
        ...
        <div>
          ...
          <div
            v-if="listSelectedItems?.length === 0"
            class="flex text-center text-gray-500 justify-center items-center min-h-[30vh]"
          >
            <p class="text-2xl" v-if="listSelectedName === 'Editlist'">
              No user other than the admin can edit this
              {{ isAutomation ? 'automation' : 'task' }} right now
            </p>
            <p class="text-2xl" v-else>
              No user is {{ listSelectedName.toLowerCase() }}ed for this
              {{ isAutomation ? 'automation' : 'task' }} right now
            </p>
          </div>
        </div>
      ...
      <div>
        ...
        <div>
          ...
          <div
            v-if="usersNotInList?.length === 0 || isAdminList(usersNotInList)"
            class="flex text-center text-gray-500 justify-center items-center min-h-[30vh]"
          >
...
async function removeUser(userEmail: string) {
  ...
  try {
    ...
    showToastMessage(
      `${userFromEmail(userEmail)?.nickname ?? 'User'} removed from the ${listSelectedName.value}.`
    )
  } finally {
    loadingOverlay.stopLoading()
  }
